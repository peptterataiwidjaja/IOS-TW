...
export const SubcontractorManager: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ========================================================================= */}
      {/* PERINGATAN DINI H-3 (3 DAYS BEFORE DEADLINE EARLY WARNING SYSTEM)        */}
      {/* ========================================================================= */}
      {subconWarnings.length > 0 && (
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-cyan-200">
                    Laporkan capaian produksi harian &amp; kendala agar dianalisis PE
                  </p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmitDailyLog} className="p-5 space-y-4 text-xs">
              ...
              <div>
                ...
                {portalTask?.subconUsername && (
                  <div className="mt-1.5 flex items-center justify-between text-[11px] bg-cyan-50/80 px-2.5 py-1.5 rounded-lg border border-cyan-200 text-cyan-900">
                    ...
                    <span>Deadline: <strong>{portalTask.estReturnDate}</strong></span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2.5 pt-1">
                <label className="block font-bold text-slate-800">
                  Status Kelancaran Produksi Hari Ini:
                </label>
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <span>Ada Masalah / Kendala</span>
                  </button>
                </div>

                {logHasIssue && (
                  <div className="p-3.5 rounded-xl bg-red-50 border border-red-200 space-y-2.5 animate-in fade-in duration-150">
                    <div>
                      <label className="block font-bold text-red-900 mb-1">Kategori Masalah Subkon *</label>
                      <select
                        value={logIssueCategory}
                        onChange={(e) => setLogIssueCategory(e.target.value as SubconIssueCategory)}
                        className="w-full p-2 rounded-lg border border-red-300 font-bold text-slate-900 bg-white"
                      >
                    ...
                    <div>
                      <label className="block font-bold text-red-900 mb-1">
                        Detail Masalah (Otomatis Mengirim Warning ke Pabrik) *
                      </label>
                      <textarea
                        rows={2}
                        value={logIssueNotes}
                        onChange={(e) => setLogIssueNotes(e.target.value)}
                        placeholder="Contoh: Dinamo mesin bordir head 3 mati atau lampu conveyor pemanas rusak..."
                        className="w-full p-2 rounded-lg border border-red-300 font-medium text-slate-900 bg-white"
              ...
              >
                ...
                <span>Simpan &amp; Kirim Laporan Target Harian</span>
              </button>
            </form>
          </div>

          {/* Right Column (7 cols): Live Analytical View & Daily History for Selected SPK */}
                    ...
                    <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                      <div
                        className={`h-full transition-all duration-300 ${
                          stats.warningMatch ? 'bg-amber-500' : 'bg-emerald-500'
                        }`}
